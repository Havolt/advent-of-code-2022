...
// Find all repeats in a string when split in half
const findRepeats = (listOfRepeats) => {
  const repeats = [];

  listOfRepeats.forEach((bag) => {
    const itemHashTable = {};
    const compartmentA = bag.slice(0, bag.length / 2).split("");
    const compartmentB = bag.slice(bag.length / 2).split("");

    compartmentA.forEach((item) => {
      if (itemHashTable[item] === undefined) {
        itemHashTable[item] = 1;
      }
    });

    for (let i = 0; i < compartmentB.length; i++) {
      if (itemHashTable[compartmentB[i]]) {
        repeats.push(compartmentB[i]);
        break;
      }
    }
  });
